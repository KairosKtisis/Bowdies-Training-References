// engine/apply_bridge_variants_bourbon_bold.js
//
// Phase 7 / Session 21 — merges bridge_variants_bourbon_bold.js into
// bottle_profiles_curated.js + redistributes corpus occurrences via
// canonical-sorted-key hash. Same shape as Session 19/20 appliers.

'use strict';
const fs = require('fs');
const path = require('path');
const { BRIDGE_VARIANTS } = require('./bridge_variants_bourbon_bold');
...
const SANDBOX_PROF = '/sessions/adoring-serene-dijkstra/mnt/outputs/bottle_profiles_curated.bourbon.js';
const SANDBOX_NOTES = '/sessions/adoring-serene-dijkstra/mnt/outputs/pairing-notes.bridge-variants-bourbon.js';
...
for (const [bottle, vs] of Object.entries(BRIDGE_VARIANTS)) {
  ...
  if (!m) { missList.push(bottle); continue; }
  const injection =
...
if (missList.length) console.log('  MISSES:', missList);
